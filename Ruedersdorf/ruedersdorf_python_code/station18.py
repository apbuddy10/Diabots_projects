from grids import GridInevitable
...
class Station18(Station):
    ...
    async def initGrids(self, ref_rack_1, ref_rack_2, r_rows, r_columns, ref_tube_1, ref_tube_2, ref_tube_3, t_rows, t_columns):
        ...
        for j in range(self.batchCount):
            cnt = 0
            initlogger.info(
                "Station 18: {0} grid  batch {1} locations:".format(self.name, j+1))
    ...
    #         for i in range(j* self.batches[j].batchSize, (j+1)*self.batches[j].batchSize):
    #             location=Station18Positions[i]
    #             tube=self.batches[j].tubes[cnt]
    #             tubeLocation=self.gridTubes.get_target_x_y(location[1] + j * t_columns)
    #             tube.location=[tubeLocation[0], tubeLocation[1], tubeLocation[2], tubeLocation[3], tubeLocation[4], tubeLocation[5]]
