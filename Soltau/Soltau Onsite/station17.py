...
class Station17(Station):
    ...
    # async def initGrids(self, ref_rack_1, ref_rack_2, r_rows, r_columns, ref_tube_1, ref_tube_2, t_rows, t_columns):
    #     self.gridRacks=Grid(ref_rack_1, ref_rack_2, ref_rack_1, r_rows, r_columns) 
    #     self.gridTubes=Grid(ref_tube_1, ref_tube_1, ref_tube_2, t_rows, t_columns)
    #     rack_dist_x = ref_rack_2[0] - ref_rack_1[0]
    #     for j in range(self.batchCount):
    #         cnt=0
    #         initlogger.info("Station 17: {0} grid  batch {1} locations:".format(self.name,j+1))
    #         for i in range(j* self.batches[j].batchSize, (j+1)*self.batches[j].batchSize):
    #             location=Station17Positions[i]
    #             tube=self.batches[j].tubes[cnt]
    #             tubeLocation=self.gridTubes.get_target_x_y(location[1])
    #             tube.location=[tubeLocation[0] + (j * rack_dist_x), tubeLocation[1], tubeLocation[2], tubeLocation[3], tubeLocation[4], tubeLocation[5]]
