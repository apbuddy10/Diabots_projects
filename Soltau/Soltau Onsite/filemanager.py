...
class FileManager:    
    ...
    async def processFiles(self,tubeResults:TubeResults):
        ...
        newBarcode = tubeResults.barCode
        ...
        newColor = tubeResults.color.value

        filename=os.path.join(self.resultsPath,"{0}.csv".format(tubeResults.barCode))
        file = open(filename, "w")
        file.write("{0};{1};{2}".format(newBarcode,tubeResults.color.name, tubeResults.plasmaLevel))
        file.close()
        jobslogger.info("CSV file writing success, {0}, {1}, {2}".format(newBarcode,tubeResults.color.name,tubeResults.plasmaLevel)) 
