...
from settings import BatchStatus, HolderStatus, Station17Positions
...
class Station18(Station):
    ...
    def initBatches(self):        
        for i in range(self.batchCount):
            batch=Stn18Batch(self.batchSize)
    ...
    #     self.gridBatch=Grid(ref_rack_1, ref_rack_2, ref_rack_1, r_rows, r_columns) 
    #     self.gridTubes=GridInevitable(ref_tube_1, ref_tube_2, ref_tube_3, t_rows, t_columns)
    #     # rack_dist_x = ref_rack_2[0] - ref_rack_1[0]
    #     rows = 5
    #     columns=10
    #     for j in range(10):
    #         cnt=0
    #         initlogger.info("Station 18: {0} grid  batch {1} locations:".format(self.name,j+1))
    #         newx = ref_tube_1[0] - (((ref_tube_1[0]-ref_tube_2[0])/(columns-1))) * j
    #         for i in range(5):
    #             if not i == 2:
    #                 newy = ref_tube_1[1] + (((ref_tube_3[1]-ref_tube_1[1])/(rows-1))) * i  
    #                 tube=self.batches[j].tubes[cnt]
    #                 tube.location=[newx,newy,ref_tube_1[2], ref_tube_1[3], ref_tube_1[4], ref_tube_1[5]]
    #                 initlogger.info("Index,location :{0}:{1}".format(cnt+1,tube.location))
    #                 cnt+=1
    #         rackLocation=self.gridBatch.get_target_x_y(j+1)
    #         initlogger.info("Rack Index,location :{0}:{1}".format(j+1,rackLocation))
    #         self.batches[j].setLocation(rackLocation)

    async def initGrids(self, ref_rack_1, ref_rack_2, ref_rack_3, r_rows, r_columns):
        self.gridRacks=Grid(ref_rack_1, ref_rack_2, ref_rack_3, r_rows, r_columns) 
        for j in range(self.batchCount):
            cnt=0
            initlogger.info("Station 18: {0} grid  batch {1} locations:".format(self.name,j+1))
            rackLocation=self.gridRacks.get_target_x_y(j+1)
            initlogger.info("Index,location :{0}:{1}".format(j+1,rackLocation))
            self.rackLocations.append(rackLocation)
            self.batches[j].setLocation(rackLocation)   
    ...
    async def fillHolder(self):
        if self.holderStatus == HolderStatus.Empty:
            jobslogger.info("Second filled racks holder made Filled")
            self.holderStatus = HolderStatus.Filled

    async def emptyHolder(self):
        if self.holderStatus == HolderStatus.Filled:
            jobslogger.info("Second filled racks holder made Empty")
            self.holderStatus = HolderStatus.Empty
    ...
    async def setTubesCountInBatches(self, stn8Batches):
        for i in range(self.batchCount):
            # stn8 first rack tubes count must be in stn18 first rack
            batch = self.batches[i]
            stn8batch = stn8Batches[i]
            batch.tubesCount = await stn8batch.getTubesCount()
            batch.status = BatchStatus.Filled
            jobslogger.info("{0}, Tubes in batch {1} : {2}".format(self.name, i+1, batch.tubesCount))
