...
class DobotApi:
    ...
    def send_data(self, string):
        # self.log(f"Send to 192.168.0.5:{self.port}: {string}")
    ...
    def wait_reply(self):
        ...
        # self.log(f'Receive from 192.168.0.64:{self.port}: {data_str}')
        return data_str
...
class DobotApiDashboard(DobotApi):
    ...
    def SetCollideDrag(self):
        """
        Force to drag mode
        """
        status = 0
        # string = "SetCollideDrag({:d})".format(status)
        string = "StartDrag()"
        self.send_data(string)
        return self.wait_reply()
