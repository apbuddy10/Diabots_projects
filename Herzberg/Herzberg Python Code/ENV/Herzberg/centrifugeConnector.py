...
class CentrifugeInterface:
  ...
  async def set_rotor_position(self, position):
    try:
      if self.conn_centrifuge is not None:
        command, check1, check2 = self.get_rotor_command(position)
        result = list(await self.clear_and_write(command))
        timeout = time.time() + 30
        rotor_set = False
        while time.time() < timeout:
            res = await self.check_rotor_position()
            if res[13] == check1 or res[13] == check2:
              rotor_set = True
              break
  ...
  def get_rotor_command(self, position):
    if position == 1 :
      return SET_ROTOR_POS_1, 120, 126
    elif position == 2 :
      return SET_ROTOR_POS_2, 123, 125
    elif position == 3 :
      return SET_ROTOR_POS_3, 125, 123
    elif position == 4 :
      return SET_ROTOR_POS_4, 113, 119
